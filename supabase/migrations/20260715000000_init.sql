...
-- Automatically create a profile in public.users on auth signup
...
BEGIN
  ...
  VALUES (
    new.id,
    new.email,
    COALESCE(new.raw_user_meta_data->>'role', 'admin'), -- Defaults to admin (e.g. first user signup)
...
-- Support policy for registering the first user / self-insertion of authenticated user
CREATE POLICY insert_own_user ON users FOR INSERT TO authenticated
...
-- Support principal creating their own profile
CREATE POLICY insert_own_principal ON principals FOR INSERT TO authenticated
